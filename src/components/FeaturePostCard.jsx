function FeaturePostCard({row, col, content}) {
  ...
  return (
    <div className="feature-blog-card" style={{background: `center / cover no-repeat linear-gradient(to bottom, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.4)), url(${img})`, gridColumn: ""}}>
      <a href="#" className="category">{category}</a>
      <div className="title">{title}</div>
      <div className="by-date">
        by <a href="#"><b>Suresh Dhumal</b></a> · {date}
      </div>
    </div>
  )
}
